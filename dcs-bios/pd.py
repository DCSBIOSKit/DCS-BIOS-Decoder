...
#            self.put(ss, es, self.out_ann, [5, ['RX_WAIT_ADDRESS']])
#        
#        elif self.state == 'RX_WAIT_ADDRESS':
#            self.put(ss, es, self.out_ann, [0, ['{}'.format(byte)]])
#            self.state = 'RX_WAIT_MSGTYPE'
#            self.put(ss, es, self.out_ann, [5, ['RX_WAIT_MSGTYPE']])
#        
#        elif self.state == 'RX_WAIT_MSGTYPE':
#            self.put(ss, es, self.out_ann, [1, ['{}'.format(byte)]])
#            self.state = 'RX_WAIT_DATALENGTH'
#            self.put(ss, es, self.out_ann, [5, ['RX_WAIT_DATALENGTH']])
#        
#        elif self.state == 'RX_WAIT_DATALENGTH':
#            self.data_length = byte
#            self.put(ss, es, self.out_ann, [2, ['{}'.format(byte)]])
#            self.state = 'RX_WAIT_DATA' if self.data_length > 0 else 'RX_WAIT_CHECKSUM'
#            self.put(ss, es, self.out_ann, [5, ['{}'.format(self.state)]])
#        
#        elif self.state == 'RX_WAIT_DATA':
#            self.put(ss, es, self.out_ann, [3, ['{}'.format(byte)]])
#            self.data_length -= 1
#            if self.data_length == 0:
#                self.state = 'RX_WAIT_CHECKSUM'
#                self.put(ss, es, self.out_ann, [5, ['RX_WAIT_CHECKSUM']])
#        
#        elif self.state == 'RX_WAIT_CHECKSUM':
#            self.put(ss, es, self.out_ann, [4, ['{}'.format(byte)]])
#            self.state = 'RX_HOST_MESSAGE_COMPLETE'
#            self.put(ss, es, self.out_ann, [5, ['RX_HOST_MESSAGE_COMPLETE']])
#            self.reset()
